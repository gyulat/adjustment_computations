...
% standard error of levelling per 1 km double-run according to specs
m0 = 0.4;  % mm/km
ost = m0*sqrt(d/1000);  % 0.3048, 0.2669, 0.2860

% known misclosure of the adjusted levelling network
% the sum of simulated measurements must observe this condition:
% dh(3-6) + dh(6-9) + dh(9-3) = misclosure
misclosure = 0.4;  % mm
...
figure(1)
hist(1000*(m6-mm6), 50)
title(sprintf('Benchmark 6  %d runs', Nsim))
xlabel('height residuals (mm)')
figure(2)
hist(1000*(m9-mm9), 50)
title(sprintf('Benchmark 9  %d runs', Nsim))
xlabel('height residuals (mm)')
